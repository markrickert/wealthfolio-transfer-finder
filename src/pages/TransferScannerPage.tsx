...
function ConfettiPiece({ index }: { index: number }) {
  ...
  return (
    <span
      className="absolute -top-2.5 block h-2.5 w-1.5 rounded-sm"
      style={{
        left: `${left}%`,
        backgroundColor: color,
        transform: `rotate(${rotation}deg)`,
        animation: `wf-transfers-confetti-fall ${duration}s ease-in ${delay}s forwards`,
      }}
    />
  );
}
...
function Confetti() {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      <style>{`
        @keyframes wf-transfers-confetti-fall {
